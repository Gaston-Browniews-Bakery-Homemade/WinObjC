...
static void _dispatch_queue_insert_specific(dispatch_queue_t dq, dispatch_queue_specific_t dqs) {
    ...
    dispatch_queue_specific_list dqsl = dq->dqsl;
...
void dispatch_queue_set_specific(dispatch_queue_t queue, const void *key, void *_Nullable context, dispatch_function_t _Nullable destructor) {
    ...
    dispatch_queue_specific_t specific;

    specific = calloc(1, sizeof(struct dispatch_queue_specific_s));
    specific->key = key;
    ...
    _dispatch_queue_add_specific(queue, specific);
    // should be it...
}

void* dispatch_queue_get_specific(dispatch_queue_t queue, const void *key) {
    dispatch_queue_specific_list dqsl = queue->dqsl;
    // foreach tailq head
    TAILQ_FOREACH(var, &dqsl->contextList, specific) {
        if(var->key == key) {
            return var->context;
...
void* dispatch_get_specific(const void *key) {
    ...
    return dispatch_queue_get_specific(dispatch_queue_get_current, key);
}